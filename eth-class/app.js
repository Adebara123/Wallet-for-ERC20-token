...
function tokenTemplateUpdate(name, symbol, totalSupply, _balance) {
...
async function getTokenDetails() {
  ...
  try {
    const [name, symbol, totalSupply, balance] = await Promise.all([
      ...
      token.symbol(),
      token.totalSupply(),
      token.balanceOf("0x85f20a6924A61904AB44243C7e2c771B3bE46734")
    ]);
    console.log(balance);
...
async function InitData() {
  const { name, symbol, totalSupply, balance } = await getTokenDetails();
  console.log(balance);
  const template = tokenTemplateUpdate(name, symbol, totalSupply / 10 ** 18, balance);
